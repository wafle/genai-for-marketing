...
from metadata_aux_data import get_metadata_data
...
def create_and_populate_customers(num_customers: int = 50000) -> List[Dict]:
    from customers_aux_data import channel, locations
    ...
    customers_date_of_birth = [
        datetime(1924, 1, 1) + timedelta(days=int(rng.integers(0, 100 * 365)))
    ...
    customers_last_activity_date = customers_last_purchase_date - rng.integers(
        10, 20, size=(num_customers)
    )
    ...
    for i in range(num_customers):
        ...
        customer["date_of_birth"] = customers_date_of_birth[i].strftime("%Y-%m-%d")
        customer["is_media_follower"] = bool(customers_is_media_follower[i])

        customer["last_sign_up_date"] = baseline_datetime - timedelta(
            days=int(customers_last_sign_up_date[i])
        )
        customer["last_sign_up_date"] = customer["last_sign_up_date"].strftime(
            "%Y-%m-%d"
        )

        customer["last_purchase_date"] = baseline_datetime - timedelta(
            days=int(customers_last_purchase_date[i])
        )
        customer["last_purchase_date"] = customer["last_purchase_date"].strftime(
            "%Y-%m-%d"
        )

        customer["last_activity_date"] = baseline_datetime - timedelta(
            days=int(customers_last_activity_date[i])
        )
...
def create_and_populate_events(num_customers: int = 50000) -> Dict:
    from events_aux_data import event_type

    events_per_customer = list(
        map(int, np.absolute(np.floor(rng.normal(1, 1, size=(num_customers)) * 100)))
    )
    num_events = sum(events_per_customer)

    events_type = rng.choice(event_type, size=(num_events))

    baseline_datetime = datetime.now()
    ...
    for i in range(num_customers):
        for _ in range(events_per_customer[i]):
            ...
            event["customer_id"] = i
            event["event_id"] = idx
            event["event_date"] = baseline_datetime - timedelta(
                days=int(events_date_delta[idx])
            )
...
def create_and_populate_transactions(num_customers: int = 50000) -> Dict:
    from transactions_aux_data import product_name, transaction_type

    transactions_per_customer = list(
        map(int, np.absolute(np.floor(rng.normal(1, 1, size=(num_customers)) * 100)))
    )
    ...
    for i in range(num_customers):
        for _ in range(transactions_per_customer[i]):
            ...
            transaction["transaction_type"] = transaction_type_choice[transaction_id][
                "transaction_type"
            ]
            ...
            transaction["transaction_date"] = baseline_datetime - timedelta(
                days=int(transactions_date_delta[transaction_id])
            )
            transaction["transaction_date"] = transaction["transaction_date"].strftime(
                "%Y-%m-%d"
            )

            transaction["product_name"] = product_name_choice[transaction_id][
                "product_name"
            ]
            transaction["product_id"] = product_name_choice[transaction_id][
                "product_id"
            ]
...
def generate_and_populate_dataset(PROJECT_ID: str, DATASET_ID: str):
    ...
    for table_id, table_schema in zip(
        ["customers", "events", "transactions", "metadata"],
...
generate_and_populate_dataset(
    "vertex-ai-builder-demo", "augmented_marketing_test_dataset"
)
